...
@section('content')
    <div class="containerDash">
        ...
        <div class="row mt-2">
            ...
            <hr class="col-12">
            <div
                class="col-12 d-flex justify-content-center align-items-center border bg-white text-secondary rounded-3 py-2">
                <span>
        ...
        @if (count($messages) > 0)
            <div class="row mt-3 mb-5 messSec rounded pointer p-3"
                onclick="window.location=`{{ route('admin.messages.index') }}`">
                @forelse ($messages as $message)
                    <div
                        class="bg-white rounded-3 my-2 p-2 col-12 messCard d-flex justify-content-between align-items-center">
                        <div class="w-3">
        ...
        @else
            <div class="row bg-white my-3">
                <span class="text-center col-12 py-3 text-secondary">You don't have any new messages to read</span>
            </div>
        @endif
        ...
        @if (count($sponsApartments) > 0)

        <div class="d-flex gap-3 px-1 sponsSec">
                @foreach ($sponsApartments as $apartment)
                    <div onclick="window.location=`{{ route('admin.apartments.show', $apartment) }}`"
                        ...
                        <div class="w-100">
                            <img src="{{ asset('/storage') . '/' . $apartment->image }}"
                                class="object-fit-cover img-card w-100 img-fluid rounded pointer"
                                alt="{{ $apartment->image }}">
                        </div>
